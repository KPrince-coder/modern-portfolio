...
const Button = forwardRef<HTMLButtonElement, ButtonProps>(
  (
    ...
    const variantClasses = {
      ...
      outline:
        "border-solid border-1 border-indigo-600 text-indigo-600 dark:text-indigo-400 hover:bg-indigo-50 dark:hover:bg-indigo-900/30",
      ghost:
    ...
    // If href is provided but button is disabled, render a button that looks like a link
    if (href && isDisabled) {
      return (
        <button className={classes} disabled={true} type="button">
          {content}
        </button>
      );
    }
